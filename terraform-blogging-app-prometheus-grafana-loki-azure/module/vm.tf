...
#  count                 = var.vm_count_rabbitmq
  ...
  depends_on = [azurerm_managed_disk.disk_sonarqube, azurerm_linux_virtual_machine.azure_vm_loki[0], azurerm_linux_virtual_machine.azure_vm_loki[1], azurerm_linux_virtual_machine.azure_vm_loki[2]]
}
...
resource "null_resource" "azurevm_sonarqube" {
  ...
  depends_on = [azurerm_linux_virtual_machine.azure_vm_sonarqube]
